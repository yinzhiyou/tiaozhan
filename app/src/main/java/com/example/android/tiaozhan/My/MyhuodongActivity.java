...
//                    qiandao();
                    ...
                    if (tag == 1){
                        showNormalDialog(i, "中途退出，您会被记违约次数一次，技术分按输扣除并均分至其他未退出方！您应得的打赏费退还至发布者，您预付的场地费支付给场馆方。", "3");
                        /*if (data.get(i).getComplaint() == 4||data.get(i).getComplaint() == 3||data.get(i).getComplaint() == 1){
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Exception e, int id) {
                       // ToastUitl.longs("网络繁忙");
                        LogU.Ld("1608", "我的活动" + e+"==="+e.getMessage());
                    }
                    ...
                    public void onResponse(String response, int id) {
                        ...
                        SPUtils.put(MyhuodongActivity.this,"MyhuodongActivity.XX",result);
                        Boolean a = result.indexOf("2000") != -1;
                        if (a) {
                            Gson gson = new Gson();
                            ...
                            listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                ...
                                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                    ...
                                    if (data.get(position-1).getReserve().equals("1")){
                                        intent.setClass(MyhuodongActivity.this, HomeReserveCGDetailsActivity.class);
...
//                            }
    ...
    //弹窗取消报名 发布 预订
    private void quxiaoCG(final int i, String biaoti, final String tag) {
        dialog = new Dialog(this, R.style.BottomDialog);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE); // 设置Content前设定
        RelativeLayout sport;
        sport = (RelativeLayout) LayoutInflater.from(this).inflate(
...
//        myOrientationListener.start();
    ...
    protected void onResume() {
        String jBxx = (String) SPUtils.get(MyhuodongActivity.this, "MyhuodongActivity.XX", "");
        if (NetUtil.getNetWorkStart(MyhuodongActivity.this)==1){
            if (!EmptyUtils.isStrEmpty(jBxx)){
                Gson gson = new Gson();
                MyHDEntity entity = gson.fromJson(jBxx, MyHDEntity.class);
                List<MyHDEntity.DataBean.PublicLstBean> list;
                list = entity.getData().getPublicLst();
//                            data.clear();
//                            data.addAll(list);
