...
public class MyTimerTaskLV extends TimerTask {
    float a;
    float b;
    final int c;
    final Timer timer;
    ...
    MyTimerTaskLV(LoopViewLV loopview, int i, Timer timer) {
        super();
    ...
    public final void run() {
        if (a == 2.147484E+09F) {
            a = (float) (c - LoopViewLV.getSelectItem(loopView)) * loopView.l * (float) loopView.h;
            if (c > LoopViewLV.getSelectItem(loopView)) {
                b = -1000F;
            } else {
                b = 1000F;
            }
