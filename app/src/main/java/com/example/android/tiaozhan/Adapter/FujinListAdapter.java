...
public class FujinListAdapter extends BaseAdapter {
    ...
    public FujinListAdapter (Context context,List<FujinEntity.DataBean.LstBean> list,String token){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (list.get(position).getIsFriend() == 0){
            viewHolder.jiahaoyou.setVisibility(View.VISIBLE);
        }else{
            viewHolder.jiahaoyou.setVisibility(View.GONE);
        }
        ...
        if (list.get(position).getUserInfo().getAge()>100){
            viewHolder.nianling.setText("年龄:");
        }else{
            viewHolder.nianling.setText("年龄:"+list.get(position).getUserInfo().getAge()+"岁");
        }

        if (list.get(position).getUserInfo().getWeight()<=0){
            viewHolder.tizhong.setText("体重:");

        }else {
            viewHolder.tizhong.setText("体重:"+list.get(position).getUserInfo().getWeight()+"kg");
        }

        if (list.get(position).getUserInfo().getTall()<=0){
            viewHolder.shengao.setText("身高:");
        }else{
